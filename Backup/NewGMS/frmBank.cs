...
namespace NewGMS
{
    public partial class frmBank : Form
    {
        ...
        string zeroAmt="0.00";
        string receipt = "Bank Receipt", payment = "Bank Payment";
        ...
        private void FillPurchaseDetails()
        {
            if (cmbRefDocDate.Text != "" && cmbRefDocNo.Text != "")
            {
                ...
                DateTime d =(DateTime)cmbRefDocDate.SelectedValue;
       ...
       private void txtAccount_TextChanged(object sender, EventArgs e)
        ...
        private void enterData()
        {
            ...
            else
            {
                ...
                dgvBankPaymentVoucher.Rows[x - 1].Cells["Column14"].Value = txtChequeNo.Text;
            }
            if (txtDebit.Text != "")
            {
        ...
        private bool ChkVoucherNo(string s)
        {
            bool val = true ;
            string vtyp = "";
            if (s == "Bank Payment")
            ...
            sda.SelectCommand.Parameters.AddWithValue("@vtyp", vtyp);
            DataSet ds = new DataSet();
            sda.Fill(ds);
        ...
        private void DeleteLedgerDTL()
        {
            ...
            if (cmbBank.Text == "Bank Payment")
            {
                cmd.Parameters.AddWithValue("@vtype", "BP");
            }
            else if (cmbBank.Text == "Bank Receipt")
            {
                cmd.Parameters.AddWithValue("@vtype", "BR");
            }
            cmd.ExecuteNonQuery();
            con.Close();
        }
        private void FillGridForDailyAccMaster()
        {
            ...
            if (cmbBank.Text == "Bank Payment")
            {
                sda.SelectCommand.Parameters.AddWithValue("@vtyp", "BP");
            }
            else if (cmbBank.Text == "Bank Receipt")
            {
                sda.SelectCommand.Parameters.AddWithValue("@vtyp", "BR");
            }
            DataSet ds = new DataSet();
            sda.Fill(ds);
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (txtMode.Text == newMode)
            {
                ...
                {
                    if (ChkVoucherNo(cmbBank.Text) == true)
                    {
        ...
        private void InsertLedgerDTL()
        {
            ...
            for (int i = 0; i < dgvBankPaymentVoucher.Rows.Count; i++)
            {
                ...
                if (cmbBank.Text == "Bank Payment")
                {
                    cmd.Parameters.AddWithValue("@vtyp", "BP");
                }
                else if (cmbBank.Text == "Bank Receipt")
                {
                    cmd.Parameters.AddWithValue("@vtyp", "BR");
                }
        ...
        private void InsertLedgerHD()
        {
            ...
            if (cmbBank.Text == payment)
            {
                cmd.Parameters.AddWithValue("@type", "BP");
            }
            else if (cmbBank.Text == receipt)
            {
                cmd.Parameters.AddWithValue("@type", "BR");
            }
        ...
        //private void UpdateAccountMasterAmt()
        //{
        //    SqlConnection con = new SqlConnection();
        //    con.ConnectionString = clsDbForReports.constr;
        //    con.Open();
        //    SqlCommand cmd = new SqlCommand("", con);
        //    cmd.CommandText = "UpdateAmtInAccountMaster";
        //    cmd.CommandType = CommandType.StoredProcedure;
        //    for (int i = 0; i < dgvBankPaymentVoucher.Rows.Count; i++)
        //    {
        //        cmd.Parameters.AddWithValue("@acc", dgvBankPaymentVoucher.Rows[i].Cells["Column10"].Value);
        //        cmd.Parameters.AddWithValue("@debit", dgvBankPaymentVoucher.Rows[i].Cells["Column6"].Value);
        //        cmd.Parameters.AddWithValue("@credit", dgvBankPaymentVoucher.Rows[i].Cells["Column7"].Value);
        //        cmd.ExecuteNonQuery();
        //        cmd.Parameters.Clear();
        //    }
        //    con.Close();
        //}
        //private void ReverseAccountMasterAmt()
        //{
        //    SqlConnection con = new SqlConnection();
        //    con.ConnectionString = clsDbForReports.constr;
        //    con.Open();
        //    SqlCommand cmd = new SqlCommand("", con);
        //    cmd.CommandText = "ReverseAmtInAccountMaster";
        //    cmd.CommandType = CommandType.StoredProcedure;
        //    for (int i = 0; i < dgvCredDeb.Rows.Count; i++)
        //    {
        //        cmd.Parameters.AddWithValue("@acc", dgvCredDeb.Rows[i].Cells["ColAcc"].Value);
        //        cmd.Parameters.AddWithValue("@debit", dgvCredDeb.Rows[i].Cells["ColDeb"].Value);
        //        cmd.Parameters.AddWithValue("@credit", dgvCredDeb.Rows[i].Cells["ColCre"].Value);
        //        cmd.ExecuteNonQuery();
        //        cmd.Parameters.Clear();
        //    }
        //    con.Close();
    ...
    private void UpdateLedgerHD()
        ...
        private void tLedgerHDInGrid()
        {
            ...
            con.ConnectionString = clsDbForReports.constr;
            SqlDataAdapter sda = new SqlDataAdapter("", con);
            sda.SelectCommand.CommandText = "select voucherno,convert(nvarchar(10),voucherdate,103) voucherdate,Amount,'Bank Payment' v from tLedgerHD where vouchertype=@vtype1 union select voucherno,convert(nvarchar(10),voucherdate,103) voucherdate,Amount,'Bank Receipt' v from tLedgerHD where vouchertype=@vtype2";
            sda.SelectCommand.Parameters.AddWithValue("@vtype1", "BP");
            sda.SelectCommand.Parameters.AddWithValue("@vtype2", "BR");
            DataSet ds = new DataSet();
            sda.Fill(ds);
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                dgvBankPayHD.Rows[i].Cells["ColBank"].Value=ds.Tables[0].Rows[i]["v"];
            }
        }
        ...
        private void CellDoubleClick(int k)
        {
            ...
            if (k != -1)
            {
                ...
                cmbBank.Text = dgvBankPayHD.Rows[k].Cells["ColBank"].Value.ToString();
                string vt = "";
                if (dgvBankPayHD.Rows[k].Cells["ColBank"].Value.ToString() == payment)
                {
                    vt = "BP";
                }
                else if (dgvBankPayHD.Rows[k].Cells["ColBank"].Value.ToString() == receipt)
                {
                    vt = "BR";
                }
                dgvBankPaymentVoucherFillFromDB(no, vt);
        ...
        private void dgvBankPaymentVoucherFillFromDB(int no, string vt)
        {
            SqlConnection con = new SqlConnection();
            con.ConnectionString = clsDbForReports.constr;
            SqlDataAdapter sda = new SqlDataAdapter("", con);
            ...
            sda.SelectCommand.Parameters.AddWithValue("@vtype", vt);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            dgvBankPaymentVoucher.Rows.Clear();
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
