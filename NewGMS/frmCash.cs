...
namespace NewGMS
{
    public partial class frmCash : Form
    {
        ...
        int RowIndex = -1;
        string payment = "Cash Payment",receipt="Cash Receipt";
        string editMode = "Edit", newMode = "New";
        string zeroamt = "0.00";
        char plus = '+', minus = '-';
       ...
       private void txtAccount_TextChanged(object sender, EventArgs e)
        ...
        private bool dataEntryCheck()
        {
            txtMsg.Text = "";
            bool dataflag = false;
            if (txtAccount.Text.Trim() == "" || cmbRefDocNo.Text.Trim()=="" ||txtDebit.Text.Trim() == "" || txtCredit.Text.Trim() == "" || txtNarration.Text.Trim() == "" || txtCostCentre.Text.Trim() == "")
            {                             
        ...
        private void DeleteLedgerDTL()
        {
            ...
            {
                cmd.Parameters.AddWithValue("@vtype", "CP");
            }
            else if (cmbBank.Text == receipt)
            {
                cmd.Parameters.AddWithValue("@vtype", "CR");
            }
            cmd.ExecuteNonQuery();
            con.Close();
        }
        private void FillGridForDailyAccMaster()
        {
            ...
            if (cmbBank.Text == payment)
            {
                sda.SelectCommand.Parameters.AddWithValue("@vtyp", "CP");
            }
            else if (cmbBank.Text == receipt)
            {
                sda.SelectCommand.Parameters.AddWithValue("@vtyp", "CR");
            }
            DataSet ds = new DataSet();
            sda.Fill(ds);
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        ...
        private void InsertLedgerDTL()
        {
            ...
            for (int i = 0; i < dgvBankPaymentVoucher.Rows.Count; i++)
            {
                ...
                if (cmbBank.Text == "Cash Payment")
                {
                    cmd.Parameters.AddWithValue("@vtyp", "CP");
                }
                else if (cmbBank.Text == "Cash Receipt")
                {
                    cmd.Parameters.AddWithValue("@vtyp", "CR");
                }
        ...
        private void tLedgerHDInGrid()
        {
            ...
            con.ConnectionString = clsDbForReports.constr;
            SqlDataAdapter sda = new SqlDataAdapter("", con);
            sda.SelectCommand.CommandText = "select voucherno,convert(nvarchar(10),voucherdate,103) voucherdate,Amount,'Cash Payment' v from tLedgerHD where vouchertype=@vtype1 union select voucherno,convert(nvarchar(10),voucherdate,103) voucherdate,Amount,'Cash Receipt' v from tLedgerHD where vouchertype=@vtype2";
            sda.SelectCommand.Parameters.AddWithValue("@vtype1", "CP");
            sda.SelectCommand.Parameters.AddWithValue("@vtype2", "CR");
            DataSet ds = new DataSet();
            sda.Fill(ds);
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                dgvBankPayHD.Rows[i].Cells["ColCash"].Value=ds.Tables[0].Rows[i]["v"];
            }
        }
        ...
        private void CellDoubleClick(int k)
        {
            ...
            if (k != -1)
            {
                ...
                cmbBank.Text = dgvBankPayHD.Rows[k].Cells["ColCash"].Value.ToString();
                string vt = "";
                if (dgvBankPayHD.Rows[k].Cells["ColCash"].Value.ToString() == payment)
                {
                    vt = "CP";
                }
                else if (dgvBankPayHD.Rows[k].Cells["ColCash"].Value.ToString() == receipt)
                {
                    vt = "CR";
                }
        ...
        private void dgvBankPaymentVoucherFillFromDB(int no, string vt)
        {
            ...
            con.ConnectionString = clsDbForReports.constr;
            SqlDataAdapter sda = new SqlDataAdapter("", con);
            sda.SelectCommand.CommandText = "select A.slno,B.AccName acc, convert(nvarchar(10),A.refDate,103) rdt,A.refNo,A.db,A.cr,E.AccName InfoAccName,A.narration,A.costcentrecode,A.acccode accode,A.InformationAccount,A.vouchertype from tLedgerDTL A left join tAccountMaster B on B.AccCode=A.acccode left join tAccountMaster E on E.AccCode=A.InformationAccount where voucherno=@voucherno and vouchertype=@vtype";
